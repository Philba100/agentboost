...
export default function SiteHeader() {
  return (
    <nav className="flex justify-between items-center px-6 md:px-12 py-5 border-b border-white/[0.08] sticky top-0 bg-[#0f172a]/90 backdrop-blur-xl z-50">
      {/* Brand Logo */}
      ...
      <div className="hidden md:flex items-center gap-8 text-[12px] font-semibold text-white/50 uppercase tracking-[0.15em]">
        ...
        <Link href="/dashboard" className="hover:text-white transition-colors lowercase font-mono tracking-normal">
          auth.init()
        </Link>
      </div>
      ...
      <div className="flex items-center gap-3">
        <Link
          href="/dashboard"
          className="bg-white/5 border border-white/10 text-white px-5 py-2 rounded-md text-[13px] font-semibold transition-all hover:bg-[#00ff9d] hover:!text-[#0f172a] hover:border-[#00ff9d] hover:shadow-[0_0_20px_rgba(0,255,157,0.2)]"
        >
          Provision Key
        </Link>
      </div>
